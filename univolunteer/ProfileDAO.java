...
public class ProfileDAO {
        ...
        try {
            ...
            while (rs.next() && sum < 20) {
                ...
                profiles.add(new Profile(rs.getString("username"), rs.getString("name"), rs.getString("surname"),
                        rs.getString("email"), rs.getString("comments"), rs.getString("country"),
                        rs.getString("city")));
        ...
        try {
            ...
            if (!rs.next()) {
                ...
                profile = new Profile(rs.getString("username"), rs.getString("name"), rs.getString("surname"),
                        rs.getString("email"), rs.getString("comments"), rs.getString("country"), rs.getString("city"));
     ...
     * @throws Exception
     */
    public void updateInfo(String username, String name, String surname, String country, String city, String email,
            String comments) throws Exception {

        String sql = "SELECT * FROM profile WHERE username=?";
        String sql1 = "UPDATE profile SET name = ?, surname = ?, email = ?"
                + ", comments = ?, country = ?, city = ? WHERE username = ?";
        String sql2 = "INSERT INTO profile "
                + " (username, name, surname, email, comments, country, city) VALUES (?, ? ,?, ?, ?, ?, ?)";
        Connection con = null;
        DB db = new DB();
        try {
            ...
            if (rs.next()) {
                ...
                if (username == null || username.length() < 1) {
                    stmt.setString(7, profile.getUsername());
                } else {
